...
import uia.Units.UnitFactory;
import uia.activation.ActivationFunction;
import uia.activation.Sigmoid;
import uia.learn.interfaces.IWeightUpdater;
import uia.memory.address.content.BetaSimilarity;
import uia.memory.address.content.ContentAddressing;
...
public class NTMMemory {
    ...
    public NTMMemory(int memoryHeight, int memoryWidth, int heads, ActivationFunction activationFunction) {
        this(null, memoryHeight, memoryWidth, new Head[heads],
                UnitFactory.getTensor2(memoryHeight, memoryWidth),
                null, activationFunction);
    ...
    NTMMemory(HeadSetting[] heading, int memoryHeight, int memoryWidth, Head[] heads, Unit[][] data, NTMMemory parent, ActivationFunction activationFunction) {
    ...
    public NTMMemory(HeadSetting[] heading, Head[] heads, NTMMemory memory, ActivationFunction activationFunction) {
        this(heading, memory.memoryHeight, memory.memoryWidth, memory.heads,
                UnitFactory.getTensor2(memory.memoryHeight, memory.memoryWidth), memory, activationFunction);
        ...
        for (int i = 0; i < h; i++) {
            ...
            for (int j = 0; j < memoryWidth; j++) {
                erases[j] = activationFunction.getValue(eraseVector[j].value);
                adds[j] = activationFunction.getValue(addVector[j].value);
            }
        }
    ...
    public String toString(){
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            for (int j = 0; j < data[i].length; j++) {
                s+=this.data[i][j].value+",";
            }
            s+="]\n";
        }
        s+="Head \n";
        ContentAddressing[] addressing= getContentAddressing();
        for (int i = 0; i < addressing.length; i++) {
            s+= Arrays.toString(addressing[i].content.value);
            s+="\n";
        }
