...
import uia.memory.address.GatedAddressing;
import uia.memory.address.Head;
import uia.memory.address.ShiftedAddressing;
import uia.memory.address.content.BetaSimilarity;
import uia.memory.address.content.ContentAddressing;
import uia.memory.address.content.CosineSimilarityFunction;
import uia.memory.address.content.SimilarityMeasure;
...
public class MemoryState   
{
    ...
    public MemoryState process(Head[] heads, ActivationFunction activationFunction) {
        ...
        for (int i = 0;i < headCount;i++) {
            ...
            GatedAddressing ga = new GatedAddressing(head.getGate(), ca, heading[i], activationFunction);
            ShiftedAddressing sa = new ShiftedAddressing(head.getShift(),ga, activationFunction);
            newHeadSettings[i] = new HeadSetting(head.getGamma(),sa);
            newReadDatas[i] = new ReadData(newHeadSettings[i], memory);
        }
        NTMMemory newMemory = new NTMMemory(newHeadSettings, heads, memory, activationFunction);
